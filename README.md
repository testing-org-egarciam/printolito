...
*   **GitOps Delivery:** ArgoCD deploys this chart to the `printolito-prod` spoke cluster, pulling environment-specific credentials and Bitnami image tag overrides (like forcing `latest` tags) from the `gitops-config` repository.
